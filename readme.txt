...
Для хранилища сетки был выбран потоконебезопасный HashMap, т.к. не предусмотрено изменение сетки во время работы приложения (сетка сохраняется только при инициализации сервиса). Для данного хранилища больше подходит двумерный массив (или ArrayList<ArrayList<>>), т.к. можно заранее указать сколько будет в нем элементов, что обеспечит константную сложность поиска элемента в хранилище. 
...
Отсутствует начальная конфигурация HashMap и ConcurrentHashMap.
Некорректное хранилище для гео-меток. 
...
Ο(n+m) - в худшем случае, когда hashCode для всех значений в хранилищах меток и гео-точек одинаковый (в каждом из хранилищ). n и m - количество меток и гео-точек.
2. Статистика по ячейке сетки. 
...
Новый вариант географической сетки
Для достижение близкого размера ячеек сетки предлагаю разбить дополнительно каждый элемент ячейки так, чтобы наименьшие элементы были бы примерно равны.  
При разбиении задать дополнительно для x и y фактор разбиения. В хранилище для каждой большой ячейки добавить двумерный массив с внутренниси ячейками и их погрешностью.
Преобразование координат в ячейку будет происходить следующем образом:
Получаем узел большой сетки, аналогично текущему решению. Далее берем степень разбиения, умножаем на дробную часть, оставляем целую часть и получаем индекс во втором массиве （двумерном）. 
tileParts [(int)((lon - (（int）lon) )*tileXFactor)][(int)((lat - (（int）lat) )*tileYFactor)]
Тем самым мы получим итоговую ячейку в сетке.